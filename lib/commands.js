...
const COMMANDS = {
  ...
  'user-navigate': {
    kind: 'special',
    argSpec: [],
    help: 'INTERACTIVE：改 URL 切 user-analysis 子 tab（不模拟点击）。支持 --to-action attr|activity_analysis_page、--clear 回用户增长',
    pages: ['user-analysis'],
    defaultPage: 'user-analysis',
  },
  ...
  'content-trend': {
    ...
    help: '日趋势（scene=9999 为合计）——走 get_article_stat_tendency_and_source XHR；传 --msgid 切单篇',
    pages: ['content-analysis'],
    defaultPage: 'content-analysis',
  },
  'content-sources': {
    ...
    help: '时段聚合的来源/渠道分布（复用同一 XHR 响应）；传 --msgid 切单篇',
    pages: ['content-analysis'],
    defaultPage: 'content-analysis',
  },
  'content-tables-dump': {
  ...
  // v0.4 内部踩点工具：不进 skill.contract.js（不暴露给 AI），仅 CLI 调试用
  'content-detail-dump': {
    kind: 'call',
    api: 'detailDomDump',
    argSpec: [],
    toArgs: (o) => [{
      ...
      textLimit: o.rowLimit ? Number(o.rowLimit) : null,
      collectAnchors: !!o.anchors,
      includeOuterHTML: !!o.outerHtml,
    }],
    help: '[内部踩点] detailpage / 任意子页 DOM 节点结构快照',
    pages: ['content-analysis'],
    defaultPage: 'content-analysis',
  },
  'content-xhr-log': {
    ...
    api: 'xhrLog',
    argSpec: [],
    toArgs: (o) => [{ limit: o.limit ? Number(o.limit) : null, filter: o.filter || null }],
    help: '[内部踩点] 已发过的 appmsganalysis ajax 请求历史（performance resource buffer）',
    pages: ['content-analysis'],
    defaultPage: 'content-analysis',
  },
...
function parseArgv(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--json') opts.json = true;
    ...
    else if (a === '--row-limit' || a === '--rows') opts.rowLimit = argv[++i];
    else if (a.startsWith('--row-limit=')) opts.rowLimit = a.slice('--row-limit='.length);
    else if (a.startsWith('--rows=')) opts.rowLimit = a.slice('--rows='.length);
    else if (a === '--filter') opts.filter = argv[++i];
    else if (a.startsWith('--filter=')) opts.filter = a.slice('--filter='.length);
    else if (a === '--anchors' || a === '--with-anchors') opts.anchors = true;
    else if (a === '--outer-html' || a === '--with-outer-html') opts.outerHtml = true;
      ...
      // 显式拒绝未识别的 flag，避免静默吞掉变成 positional（v0.3.1 之前的 UX 坑）。
      const err = new Error(
...
function printHelp() {
  ...
  lines.push(
    ...
    '  --to   YYYY-MM-DD   结束日期',
    '  --limit <n>      结果条数上限',
    '  --rows <n>       每张表保留多少行（content-tables-dump，别名 --row-limit）',
    '  --filter <regex> content-xhr-log 的 URL 正则过滤器（默认匹配 mp.weixin.qq.com/misc/appmsganalysis）',
    '  --anchors, --with-outer-html  content-detail-dump 专用：附带 [a[href]] 锚点列表 / 节点 outerHTML',
    '  --to-action, --to-type, --to-front-type, --msgid, --publish-date, --clear',
    '                   content-navigate / user-navigate 专用：改 URL 参数（INTERACTIVE 档位，不模拟点击）',
    '                   content-trend / content-sources 也接受 --msgid / --publish-date（v0.4，单篇维度）',
    ...
    '  node index.js content-trend --range 7d --msgid 2247484081_1 --publish-date 2026-04-14   # v0.4: 单篇维度趋势',
    '  node index.js content-sources --range 7d',
    '  node index.js content-sources --range 7d --msgid 2247484081_1 --publish-date 2026-04-14 # v0.4: 单篇维度来源',
    '  node index.js content-tables-dump --limit 5 --rows 3',
    '  node index.js content-navigate --to-action report --from 2026-04-01 --to 2026-04-23',
    '  node index.js content-navigate --to-action all --to-front-type without_notice    # v0.4: front_type 白名单',
    '  node index.js content-navigate --msgid 2247484081_1 --publish-date 2026-04-14',
    '  node index.js user-navigate --to-action attr           # v0.5: 切到用户属性 tab',
    '  node index.js user-navigate --clear                    # v0.5: 回用户增长 tab',
    '  # v0.4 内部踩点（不进 AI tool 列表）：',
    '  node index.js content-detail-dump --anchors --limit 30',
    '  node index.js content-xhr-log --filter appmsganalysis',
    '',
    '注意:',
